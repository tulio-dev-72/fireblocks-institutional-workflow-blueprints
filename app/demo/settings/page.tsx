...
export default function SettingsPage() {
  ...
  const [policyRestriction, setPolicyRestriction] = useState<AccessRestrictionDetails | null>(null);
        ...
        <Card variant="elevated">
          ...
          <PrimaryButton
            ...
            onClick={async () => {
              ...
              setPolicyError(null);
              setPolicyRestriction(null);
              const result = await updatePolicy({ approvalThreshold: nextThreshold });
              if (!result.ok) {
                if (result.restriction) {
                  setPolicyRestriction(result.restriction);
                }
          ...
          {policyRestriction ? (
            ...
            <p className="mt-3 text-xs text-ops-danger">{policyError}</p>
          ) : null}
        </Card>
              ...
              >
                ...
                <DangerButton
                  onClick={async () => {
                    setPolicyError(null);
                    setPolicyRestriction(null);
                    const result = await removeWhitelistAddress(address);
                    if (!result.ok) {
                      if (result.restriction) {
                        setPolicyRestriction(result.restriction);
                      }
          ...
          <div className="mt-3">
            ...
            <SecondaryButton
              ...
              onClick={async () => {
                ...
                setPolicyError(null);
                setPolicyRestriction(null);
                const result = await addWhitelistAddress(newAddress.trim());
                if (!result.ok) {
                  if (result.restriction) {
                    setPolicyRestriction(result.restriction);
                  }
