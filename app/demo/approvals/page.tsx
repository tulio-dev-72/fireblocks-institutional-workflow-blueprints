    ...
    try {
      ...
      if (!approved.ok) {
        if (approved.restriction) {
          setAccessRestriction(approved.restriction);
        }
        throw new Error(approved.error || "Authorization state could not be saved. Settlement remains pending.");
      }

      setAuthorizedTxId(result.fireblocksTxId || null);
      setDemoFallback(result.demoMode);
      trackProductEvent("approval_action", {
        page: "/demo/approvals",
        action: "approve",
        role: effectiveRole ?? "unknown",
        workflow_type: "settlement_authorization",
        status: "approved",
      });
