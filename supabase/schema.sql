...
-- Role helper for RLS (matches app RBAC: analyst, treasury_manager, admin)
...
drop policy if exists "Role-scoped settlement read" on public.settlement_requests;
create policy "Role-scoped settlement read"
  on public.settlement_requests for select to authenticated
  using (
    public.current_user_role() in ('treasury_manager', 'admin')
    or created_by = auth.uid()
  );

drop policy if exists "Authenticated users can create settlements" on public.settlement_requests;
drop policy if exists "Analysts can create settlements" on public.settlement_requests;
create policy "Analysts can create settlements"
...
drop policy if exists "Treasury managers can update settlements" on public.settlement_requests;
create policy "Treasury managers can update settlements"
  on public.settlement_requests for update to authenticated
  using (public.current_user_role() = 'treasury_manager')
  with check (public.current_user_role() = 'treasury_manager');

-- Approvals
drop policy if exists "Authenticated users can read approvals" on public.approvals;
drop policy if exists "Managers and admins can read approvals" on public.approvals;
create policy "Managers and admins can read approvals"
  on public.approvals for select to authenticated
  using (public.current_user_role() in ('treasury_manager', 'admin'));

drop policy if exists "Authenticated users can create approvals" on public.approvals;
drop policy if exists "Treasury managers can create approvals" on public.approvals;
create policy "Treasury managers can create approvals"
  on public.approvals for insert to authenticated
  with check (
    approver_id = auth.uid()
    and public.current_user_role() = 'treasury_manager'
  );
...
drop policy if exists "Operational roles can insert audit logs" on public.audit_logs;
create policy "Operational roles can insert audit logs"
  on public.audit_logs for insert to authenticated
  with check (public.current_user_role() in ('analyst', 'treasury_manager', 'admin'));

-- Fireblocks events (webhooks use service role; lifecycle sync via treasury_manager API)
drop policy if exists "Authenticated users can read fireblocks events" on public.fireblocks_events;
drop policy if exists "Admins can read fireblocks events" on public.fireblocks_events;
create policy "Admins can read fireblocks events"
  on public.fireblocks_events for select to authenticated
  using (public.current_user_role() = 'admin');

drop policy if exists "Authenticated users can insert fireblocks events" on public.fireblocks_events;
drop policy if exists "Managers can insert fireblocks events" on public.fireblocks_events;
create policy "Managers can insert fireblocks events"
  on public.fireblocks_events for insert to authenticated
  with check (public.current_user_role() in ('treasury_manager', 'admin'));
