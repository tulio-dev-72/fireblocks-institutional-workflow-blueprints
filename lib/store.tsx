...
import { getActionAccessRestriction } from "@/lib/auth/access-restriction";
import {
  getAccessRestrictedMessage,
  resolveAccessRestrictionFromError,
...
type CreateTransferResult =
  ...
  | { ok: false; error: string; restriction?: AccessRestrictionDetails };
...
type AppContextValue = {
  ...
  rejectTransfer: (transferId: string) => Promise<ApproveRejectResult>;
  updatePolicy: (policy: Partial<PolicySettings>) => Promise<PolicyMutationResult>;
  addWhitelistAddress: (address: string) => Promise<PolicyMutationResult>;
  removeWhitelistAddress: (address: string) => Promise<PolicyMutationResult>;
  ...
  const value = useMemo<AppContextValue>(
    () => ({
      ...
      syncFireblocksTransferStatus: async (input) => {
        if (isSupabasePersistenceEnabled()) {
          try {
            ...
            // Lifecycle sync runs in background — avoid unhandled rejections.
          }
          return;
        }
      ...
      createTransfer: async (input, options) => {
        ...
        if (isSupabasePersistenceEnabled()) {
          try {
            ...
            const restriction = resolveAccessRestrictionFromError(error, "create_settlement", [
              "analyst",
            ]);
            return {
              ok: false,
              error:
                getAccessRestrictedMessage(error) ??
      ...
      approveTransfer: async (transferId, fireblocks) => {
        ...
        if (!role || !canApproveTransfers(role)) {
          return {
            ok: false,
            error: "Treasury Manager authorization is required for settlement release.",
            restriction: getActionAccessRestriction("authorize_settlement", ["treasury_manager"]),
          };
        }
        const transfer = state.transfers.find((item) => item.id === transferId);
        if (!transfer || transfer.status !== "PENDING_APPROVAL") {
          return { ok: false, error: "Settlement is not pending authorization." };
        }
        ...
        if (isSupabasePersistenceEnabled()) {
          try {
            ...
            return {
              ...
              error:
                getAccessRestrictedMessage(error) ??
                (error instanceof Error ? error.message : "Authorization could not be saved."),
              restriction:
                resolveAccessRestrictionFromError(error, "authorize_settlement", [
                  "treasury_manager",
                ]) ?? undefined,
            };
          }
      ...
      rejectTransfer: async (transferId) => {
        const role = resolveEffectiveRole(state);
        if (!role || !canRejectTransfers(role)) {
          return {
            ok: false,
            error: "Treasury Manager authorization is required to reject settlements.",
            restriction: getActionAccessRestriction("authorize_settlement", ["treasury_manager"]),
          };
        }
        const transfer = state.transfers.find((item) => item.id === transferId);
        ...
        if (isSupabasePersistenceEnabled()) {
          try {
            ...
            return {
              ...
              error:
                getAccessRestrictedMessage(error) ??
                (error instanceof Error ? error.message : "Rejection could not be saved."),
              restriction:
                resolveAccessRestrictionFromError(error, "authorize_settlement", [
                  "treasury_manager",
                ]) ?? undefined,
      ...
      updatePolicy: async (policy) => {
        ...
        if (!role || !canManagePolicy(role)) {
          return {
            ok: false,
            error: "Platform Admin privileges required for governance configuration.",
            restriction: getActionAccessRestriction("manage_governance", ["admin"]),
          };
        }
        ...
        if (isSupabasePersistenceEnabled()) {
          try {
            ...
            return {
              ...
              restriction:
                resolveAccessRestrictionFromError(error, "manage_governance", ["admin"]) ??
                undefined,
            };
          }
        }
        ...
        if (isSupabasePersistenceEnabled()) {
          try {
            ...
            return {
              ...
              error:
                getAccessRestrictedMessage(error) ??
                (error instanceof Error ? error.message : "Unable to update allowlist."),
              restriction:
                resolveAccessRestrictionFromError(error, "manage_governance", ["admin"]) ??
                undefined,
            };
