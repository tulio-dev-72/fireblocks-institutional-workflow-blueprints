...
import { resolveAccessRestrictionFromError } from "@/lib/auth/api-errors";
...
export function FireblocksIntegrationPanel() {
  ...
  const [deliveryRestriction, setDeliveryRestriction] =
    useState<AccessRestrictionDetails | null>(null);
  ...
  async function refreshStatus() {
    ...
    try {
      const [nextStatus, webhookInfo, deliveryInfo] = await Promise.all([
        ...
        fetchWebhookDeliveries().catch((error) => {
          const restriction = resolveAccessRestrictionFromError(error, "integration_status", [
            "admin",
          ]);
          if (restriction) {
            setDeliveryRestriction(restriction);
          }
          return null;
        }),
      ]);
