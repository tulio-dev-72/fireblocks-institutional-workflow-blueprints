...
export function PrimarySettlementWorkflow({ onBack }: { onBack: () => void }) {
  ...
  const [submitRestriction, setSubmitRestriction] = useState<AccessRestrictionDetails | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [authError, setAuthError] = useState<string | null>(null);
  const [authRestriction, setAuthRestriction] = useState<AccessRestrictionDetails | null>(null);
  ...
  async function handleSubmitSettlement(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!result.ok) {
      if (result.restriction) {
        setSubmitRestriction(result.restriction);
      }
  ...
  async function handleAuthorize(transferId: string) {
    ...
    try {
      ...
      if (!approved.ok) {
        if (approved.restriction) {
          setAuthRestriction(approved.restriction);
        }
        throw new Error(approved.error || "Authorization could not be saved.");
      }
  ...
  async function handleReject(transferId: string) {
    ...
    if (!result.ok) {
      if (result.restriction) {
        setAuthRestriction(result.restriction);
      }
        ...
        <form onSubmit={handleSubmitSettlement} className="space-y-3">
          ...
          {submitRestriction ? (
            <ActionAccessRestricted
              ...
              restriction={submitRestriction}
            />
          ) : null}
          {submitError && !submitRestriction ? (
            <Card variant="accent">
        ...
        <div className="space-y-3">
          ...
          {authRestriction ? (
            <ActionAccessRestricted
              ...
              requiredRoles={["treasury_manager"]}
              currentRole={effectiveRole}
              restriction={authRestriction}
            />
          ) : null}

          {authError && !authRestriction ? (
            <Card variant="accent">
